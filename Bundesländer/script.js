...
function render() {
  ...
  for (let i = 0; i < bundeslaender.length; i++) {
    ...
    content.innerHTML += generateTest(i);

    let contentCountry = document.getElementById(`idCountry${i}`);
    for (let j = 0; j < country["comments"].length; j++) {
      const comment = country["comments"][j];
      contentCountry.innerHTML += /*html*/ `
                <div>${comment}</div>
            `;
    }
  }
...
function generateTest(i) {
  let test = bundeslaender[i];
  return /*html*/ `
  <div class="row">
      <h2>${test["name"]}</h2>
      <div id="idCountry${i}"></div>
      <input id="input${i}">
      <button onclick="addNotiz(${i})">Add</button>
  </div>
`;
}
...
function addNotiz(x) {
  ...
  bundeslaender[x]["comments"].push(input.value);
  render();
}
